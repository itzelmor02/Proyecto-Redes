...
def capa_3_simulada(entidad, paquete):
    """Simula la capa de red con pérdida y corrupción de paquetes."""
    probabilidad_perdida = 0.2
    probabilidad_corrupcion = 0.1

    if random.random() < probabilidad_perdida:
        print(f"[Capa 3] El paquete de {entidad} se ha perdido.")
        return  # El paquete se pierde.

    if random.random() < probabilidad_corrupcion:
        paquete["checksum"] = random.randint(0, 65535)  # Corrompe el paquete
        print(f"[Capa 3] El paquete de {entidad} ha sido corrompido.")

    print(f"[Capa 3] Enviando paquete de {entidad}: {paquete}")
    capa_3(entidad, paquete)  # Envía el paquete a la capa correspondiente
